...
console.log(
  products.map((product) => ({
    ...product,
    timesOrdered: orders.filter(({ productId }) => productId === product.id)
      .length,
  })),
);
...
const getQuantityXTimes = (targetId) => {
  const times = orders
    .filter(({ productId }) => productId === targetId)
...
console.log(
  users.map((user) => ({
    ...
    pendingOrders: orders.filter(
      ({ status, userId }) => status === "pending" && userId === user.id,
    ).length,
  })),
);
...
console.log(
  users.map((user) => ({
    ...user,
    hasCompletedOrders: orders.some(
      ({ status, userId }) => status === "completed" && userId === user.id,
    ),
  })),
);
...
console.log(
  users.map((user) => ({
    ...
    maxOrder: orders
      .filter(({ userId }) => userId === user.id)
      .reduce(
        (maxOrder, { total }) => (maxOrder > total ? maxOrder : total),
        0,
      ),
  })),
);
...
console.log(
  inStockProducts.map((product) => ({
    ...
    pendingOrderCount: orders.filter(
      ({ status, productId }) =>
        status === "pending" && productId === product.id,
    ).length,
  })),
);
...
const getUniqueProducts = (targetId) => {
  const unique = [
    ...new Set(orders.filter(({ userId }) => userId === targetId)),
  ];
  return unique;
};
console.log(
...
console.log(
  products.map((product) => ({
    ...product,
    isPopular:
      orders.filter(({ productId }) => productId === product.id).length >= 2,
  })),
);
...
console.log(
  products.map((product) => {
    const productFilterById = orders.filter(
      ({ productId }) => productId === product.id,
    );
    return {
      ...product,
      stats: {
        orderCount: productFilterById.reduce(
...
console.log(
  students.map((student) => {
    const ave =
      student.grades.reduce((sum, x) => sum + x, 0) / student.grades.length;
    return {
      ...student,
      performance: {
...
console.log(
  orders.reduce((acc, { productId, userId, total, quantity }) => {
    ...
    acc[category] = {
      ...
      performance:
        acc[category].totalRevenue > 400
          ? "high"
          : acc[category].totalRevenue > 150
            ? "stable"
            : "low",
    };
