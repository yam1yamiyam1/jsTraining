...
console.log(
  users.reduce((count, { isActive }) => (isActive ? count + 1 : count), 0),
);
console.log(
  products.reduce((count, { inStock }) => (inStock ? count + 1 : count), 0),
);
console.log(numbers.reduce((max, x) => (max > x ? max : x)));
console.log(
  products.reduce((min, { price }) => (min > price ? price : min), Infinity),
);
...
console.log(
  products.filter(({ price }) => price < 100).map(({ price }) => price),
);
console.log(
  users.filter(({ isActive }) => isActive === false).map(({ name }) => name),
);
console.log(
  products
    .filter(({ inStock }) => inStock === false)
...
console.log(
  orders.filter(({ total }) => total > 150).map(({ status }) => status),
);
console.log(
  users
...
console.log(
  users.reduce((count, { role }) => (role === "admin" ? count + 1 : count), 0),
);
console.log(orders.filter(({ status }) => status === "pending").length);
console.log(users.reduce((max, { salary }) => (max > salary ? max : salary)));
console.log(
  products.reduce(
    (min, { quantity }) => (min > quantity ? quantity : min),
    Infinity,
  ),
);
console.log(
  students.filter(({ attendance }) => attendance > 90).map(({ name }) => name),
);
...
console.log(
  users.filter(({ role }) => role === "admin").map(({ salary }) => salary),
);
console.log(
  users
    .filter(({ age }) => age > 30)
...
console.log(
  users
    .filter(({ role }) => (role !== "admin") & (role !== "moderator"))
    .map(({ name }) => name),
);
console.log(products.filter(({ price }) => price < 80).map(({ name }) => name));
console.log(
  users
    .filter(({ role, isActive }) => (role === "admin") & (isActive === true))
    .map(({ name }) => name),
);
console.log(
  orders.filter(({ total }) => total > 100).map(({ status }) => status),
);
console.log(
  users
    .filter(({ isActive }) => isActive)
    .reduce((sum, { salary }) => sum + salary, 0) /
    users.filter(({ isActive }) => isActive).length,
);
console.log(
  users
...
console.log(
  students.map(
    ({ grades }) => grades.reduce((sum, x) => sum + x, 0) / grades.length,
  ),
);
console.log(
  users.reduce((roleCount, { role }) => {
    if (!roleCount[role]) {
      roleCount[role] = 0;
    }
    roleCount[role]++;
...
console.log(
  products.reduce((categoryCount, { category }) => {
...
console.log(
  users.filter(
    ({ salary }) =>
      salary >
      users.reduce((sum, { salary }) => sum + salary, 0) / users.length,
  ),
);
...
console.log(
  orders.reduce((orderCount, { userId }) => {
...
const userOrders = orders
  .reduce((ordersById, { userId, total }) => {
...
console.log(
  (orders.filter(({ status }) => status === "completed").length /
    orders.length) *
    100,
);
...
console.log(
  students
    .sort((a, b) => getAverage(b.grades) - getAverage(a.grades))
    .map((student, index) => {
      return {
        ...student,
        rank: index + 1,
...
console.log(
  users.reduce((salaryByDept, { role, salary }) => {
...
console.log(
  products.reduce((byCategory, { category, quantity, price }) => {
...
console.log(
  orders.reduce((byUserId, { userId, total }) => {
...
console.log(
  products
    .filter(({ quantity, inStock }) => quantity < 5 || inStock === false)
    .map(({ name }) => name),
);

console.log(
  (students.map(({ grades }) => getAverage(grades)).filter((x) => x > 80)
    .length /
    students.length) *
    100,
);
...
console.log(
  products
    .filter(({ inStock, quantity }) => (quantity > 5) & inStock)
...
console.log(
  orders.reduce((orderById, { status }, _, arr) => {
